...
export function fileTypeIcon(name: string) {
  ...
  switch (ext) {
    ...
    case 'cc':
    case 'cxx':
      return <span className={`${base} bg-indigo-600/30 text-indigo-300 border border-indigo-500/40`}>C++</span>;
    case 'h':
    case 'hpp':
      return <span className={`${base} bg-emerald-600/30 text-emerald-300 border border-emerald-500/40`}>H</span>;
    case 'txt':
      return <span className={`${base} bg-neutral-600/30 text-neutral-300 border border-neutral-500/40`}>TXT</span>;
    default:
      return <span className={`${base} bg-fuchsia-600/30 text-fuchsia-300 border border-fuchsia-500/40`}>{ext?.slice(0,3)}</span>;
  }
}
